...
def evaluate_phase1_causal_order(bundle: ArtifactBundle, refs: list[str]) -> EvalResult:
    agreements = {item["agreement_id"]: item for item in bundle.by_type("agreement")}
    revisions = {item["revision_id"]: item for item in bundle.by_type("revision")}
    events = [item for item in bundle.by_type("event") if item.get("event_type") == "REVISION_ACTIVATED"]

    if not agreements:
        return EvalResult("fail", "phase1 causal order requires at least one agreement", refs)
    if not revisions:
        return EvalResult("fail", "phase1 causal order requires at least one revision", refs)
    if not events:
        return EvalResult("fail", "phase1 causal order requires at least one revision activation event", refs)
    ...
    for event in events:
        ...
        if revision is None:
            return EvalResult(
                "fail",
                f"event {event['event_id']} references missing revision {event['revision_id']}",
                refs,
            )
        agreement = agreements.get(revision["agreement_id"])
        if agreement is None:
            return EvalResult(
                "fail",
                f"revision {revision['revision_id']} references missing agreement {revision['agreement_id']}",
                refs,
            )
        if event["agreement_id"] != agreement["agreement_id"] or event["agreement_id"] != revision["agreement_id"]:
            return EvalResult(
                "fail",
                f"event {event['event_id']} does not preserve agreement/revision linkage",
                refs,
            )
        agreement_at = to_iso_utc(agreement["created_at"])
        revision_at = to_iso_utc(revision["created_at"])
        event_at = to_iso_utc(event["created_at"])
        if not (agreement_at <= revision_at <= event_at):
            return EvalResult(
                "fail",
                f"phase1 causal order violated for agreement {agreement['agreement_id']}, revision {revision['revision_id']}, event {event['event_id']}",
                refs,
            )
    return EvalResult("pass", "phase1 agreement->revision->event order is preserved", refs)


def evaluate_phase1_event_references_revision(bundle: ArtifactBundle, refs: list[str]) -> EvalResult:
    revisions = {item["revision_id"]: item for item in bundle.by_type("revision")}
    events = [item for item in bundle.by_type("event") if item.get("event_type") == "REVISION_ACTIVATED"]

    if not events:
        return EvalResult("fail", "phase1 event linkage requires at least one revision activation event", refs)

    for event in events:
        if event["revision_id"] not in revisions:
            return EvalResult(
                "fail",
                f"event {event['event_id']} references missing revision {event['revision_id']}",
                refs,
            )
    return EvalResult("pass", "phase1 events reference existing revisions", refs)


def evaluate_phase1_revision_references_agreement(bundle: ArtifactBundle, refs: list[str]) -> EvalResult:
    agreements = {item["agreement_id"]: item for item in bundle.by_type("agreement")}
    revisions = bundle.by_type("revision")

    if not revisions:
        return EvalResult("fail", "phase1 revision linkage requires at least one revision", refs)

    for revision in revisions:
        if revision["agreement_id"] not in agreements:
            return EvalResult(
                "fail",
                f"revision {revision['revision_id']} references missing agreement {revision['agreement_id']}",
                refs,
            )
    return EvalResult("pass", "phase1 revisions reference existing agreements", refs)
...
INVARIANT_EVALUATORS: dict[str, Callable[[ArtifactBundle, list[str]], EvalResult]] = {
    ...
    "phase1_causal_order_agreement_revision_event": evaluate_phase1_causal_order,
    "phase1_event_references_existing_revision": evaluate_phase1_event_references_revision,
    "phase1_revision_references_existing_agreement": evaluate_phase1_revision_references_agreement,
}
