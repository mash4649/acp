...
最近の評価では、技術面の妥当性だけでなく、商談や導入説明で何を強く言うべきかが課題として見えました。

### 見えてきた課題

| 課題 | なぜ重要か | 次の対応 |
|------|------------|----------|
| 対象セグメントが広い | 広すぎる訴求は導入メッセージがぼやける | 最初に狙う業種・役割を絞る |
| ROI の一言が弱い | 意思決定者は短い数字で判断する | 定量結果を 1 行で言い切る |
| 既存ツール比較がない | 乗り換え・追加導入の理由が弱い | 近接ツールとの比較表を用意する |
| 小規模チーム向け導線が明示されていない | 小さい現場ほど導入摩擦に敏感 | ACP Lite を別導線として用意する |
| OSS としての入口がやや硬い | コミュニティは最初の成功体験を重視する | 軽量な入口を通常版と分ける |

### 次のロードマップ

| 手順 | 変えること | 成果物 |
|------|------------|--------|
| 1 | 最初の対象セグメントを絞る | 明確な buyer persona とユースケース |
| 2 | 比較材料を追加する | 既存ツールとの比較表 |
| 3 | 意思決定者向けの短い価値訴求を作る | 数値付きの 1 行メッセージ |
| 4 | 小規模チーム向けの ACP Lite を定義する | 1 コマンドで始められる最小版 |
| 5 | 導入経路を検証する | Pilot チェックリストと価格仮説 |

### ACP Lite（小規模開発向け）

ACP Lite は説明責任の境界を保ったまま、企業向けの重さを削る構想です。

- 1 コマンドで実行できる
- 1 つの report を確認する
- 最小限の task bundle だけを使う
- 初期設定と文書を少なくする
- day 1 ではガバナンス一式を必須にしない

単独の小さなリポジトリ、少人数の内部ツール、個人メンテナがまず説明責任を持ちたい場面に向けた入口として使います。
