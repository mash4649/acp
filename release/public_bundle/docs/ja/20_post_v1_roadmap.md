...
> **ステータス更新（2026-03-26）**: K-1/K-2/K-3/K-4/K-5、C-1/C-2/C-3/C-4/C-5、A-1/A-2/A-3/A-4、B-1/B-2、D-1/D-2/D-3/D-4/D-5、E-1/E-2/E-3/E-4、F-1/F-2/F-3、G-1/G-2/G-3、H-1/H-2/H-3、I-1/I-3 は実装済み。
...
| 領域 | 達成済み | 未着手・不足 |
|------|---------|-------------|
| **スキーマ** | Core-15 全スキーマ＋companion 9 種＋meta 2 種 | companion ベクトルは 2/9 のみカバー。スキーマ間 `$ref` なし（各自独立）。進化ルールは人間向けのみ |
| **Conformance** | Phase1/2/3 プロファイル、38 ベクトル、10 不変条件ケース、parity | Phase1 にケース無し。companion ベクトル空白大。外部ハーネス認定プログラム無し |
...
### 課題
- `15_real_world_impact.md` に日本語版がない
- `16_v1_release_execution_plan.md` に英語版がない
- SDK 統合ガイド・本番ハーネス実装ガイドがない
- `schemas/README.md` の「starter, not release-gate quality」表記が残っている

### アクション
| # | タスク | 優先度 | 成果物 |
|---|--------|--------|--------|
| E-1 | `15_real_world_impact.md` 日本語版作成 | 中 | `docs/ja/15_real_world_impact.md` |
| E-2 | `16_v1_release_execution_plan.md` 英語版作成 | 中 | `docs/en/16_v1_release_execution_plan.md` |
...
- `15_real_world_impact.md` に業種別インパクトがあるが、対応する成果物テンプレートがない

### アクション
| # | タスク | 成果物 |
|---|--------|--------|
| J-1 | 業種別テンプレート（採用 / 外注制作 / リサーチ / マーケ / 社内マルチエージェント） | `examples/templates/` |
| J-2 | 導入事例集（匿名可）の収集と公開 | `docs/*/case_studies.md` |
| J-3 | 「ACP を既存システムに1日で組み込む」チュートリアル | `docs/*/tutorial_one_day.md` |
