...
|---|---|---|---|---|
| Standards researchers | Precise boundary, replayability, interoperability | "Accountability semantics without replacing your stack." | `prompt != contract`, `claim != evidence`, `verification != settlement` + Core-15 | Review `public/en/06_comparison_matrix_short.txt` and `public/en/10_schema_release_plan.md` |
...
| AI builders and creators | Trustable collaboration and dispute handling | "When delegated AI work fails, accountability does not disappear." | Delegation edge + reservation + freeze/dispute hooks | Review `public/en/05_anchor_use_case.txt` |
| Influencers/media | Clear category and why now | "ACP is the missing accountability layer between prompts and outcomes." | Narrow core + strong invariants + companion profiles | Use `public/en/02_launch_copy_bank.md` snippets |
